/*
 * MindFlow AI - Client Side Router
...
Machine Learning lecture revision</textarea>
    ...
    function mountView(path) {
        ...
        if (path === '/') {
            breadcrumbsContainer.innerHTML = `<a href="/" class="breadcrumb-link" id="breadcrumb-home">MindFlow AI</a>`;
        } else {
            ...
            `;
            const featuresBreadcrumb = document.getElementById('breadcrumb-features');
            if (featuresBreadcrumb) {
                featuresBreadcrumb.addEventListener('click', e => {
                    e.preventDefault();
                    window.shouldScrollToFeatures = true;
                    navigateTo('/');
                });
            }
        }
    ...
    function initializePageScripts(path) {
        ...
        if (window.MindFlowEffects) {
            window.MindFlowEffects.changeTheme(path);
        }

        // Call individual javascript behaviors
        if (path === '/') {
            if (window.MindFlowLanding) {
                window.MindFlowLanding.init();
            }
            if (window.shouldScrollToFeatures) {
                window.shouldScrollToFeatures = false;
                setTimeout(() => {
            ...
            if (window.MindFlowFeatures) {
                window.MindFlowFeatures.init(path);
            }
        }
    }
    ...
    function bindRouteListeners() {
        ...
        // Special: Explore Features links scroll on Home screen
        ...
        const exploreBtnFooter = document.getElementById('explore-features-btn-footer');
        if (exploreBtnFooter) {
            exploreBtnFooter.addEventListener('click', e => {
                e.preventDefault();
                const featuresSection = document.getElementById('features-preview');
                if (featuresSection) {
                    featuresSection.scrollIntoView({ behavior: 'smooth' });
